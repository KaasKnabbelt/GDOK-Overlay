 ...
 * measured on pixel level. Run with {@code gradlew :fabric-1.21.11:runClientGameTest}.
 *
...
public class OverlayRenderGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext ctx) {
        ctx.runOnClient(mc -> {
            ...
            mc.options.getInactivityFpsLimit().setValue(net.minecraft.client.option.InactivityFpsLimit.MINIMIZED);
            mc.options.getViewDistance().setValue(8);
        });

        try (TestSingleplayerContext sp = ctx.worldBuilder()
                .adjustSettings(c -> c.setGameMode(WorldCreator.Mode.CREATIVE))
                .create()) {
            ...
            ctx.waitFor(mc -> mc.world != null
                    && mc.world.isChunkLoaded(0, 0) && mc.world.isChunkLoaded(1, 1)
                    && mc.world.isChunkLoaded(-1, -1) && mc.world.isChunkLoaded(1, -1) && mc.world.isChunkLoaded(-1, 1), 1200);
            ctx.waitTicks(100);
            ...
            sp.getServer().runCommand("tp @a 8.5 -47 8.5 0 90");
            java.util.concurrent.CompletableFuture<Void> reload = ctx.computeOnClient(mc -> mc.reloadResources());
            ...
            // 12. Eerste persoon: de arm rechtsonder moet vóór de overlay blijven (regressie op
            //     "overlay rendert over de rechterhand"). Creative met lege hand, steil omlaag
            //     kijkend op een maaiveld-overlay: het midden is magenta, de arm-regio niet volledig.
            ctx.runOnClient(mc -> {
                OverlayManager m = OverlayManager.getInstance();
                m.resetSession();
                m.addOverlay(square("paint_magenta_wool", "paint", 0, 0, 16, OVERLAY_Y, "magenta_wool", 255, 0, 255));
    ...
    private static int averageFps(ClientGameTestContext ctx, int ticks) {
        ...
        for (int i = 0; i < ticks; i += 10) {
            ctx.waitTicks(10);
            sum += ctx.computeOnClient(mc -> mc.getCurrentFps());
            samples++;
        }
